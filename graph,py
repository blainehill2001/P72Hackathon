...
class Edge:
    ...
    def Manhattan_Distance(self):
        ...
        return (abs(self.start_node.longitude - self.end_node.longitude) + abs(self.start_node.latitude - self.end_node.latitude))*100
...
class Graph:
    ...
    def add_edge(self, node1, node2):
        if node1 and node2:
            ...
            self.graph.add_edge((node1.longitude, node1.latitude), (node2.longitude, node2.latitude), weight=edge.weight)
    ...
    def update_edge(self, node1, node2, new_money, new_transport):
        ...
        self.graph.add_edge((node1.longitude, node1.latitude), (node2.longitude, node2.latitude), weight=new_edge.weight)
    ...
    def is_valid_path(self, start, end, node):
        if node.longitude >= min(start.longitude, end.longitude) and node.longitude <= (max(start.longitude, end.longitude)+1) and node.latitude >= min(start.longitude, end.longitude) and node.latitude <= (max(start.latitude, end.latitude)+1):
            return True
        return False
    
    def update_path(self,start, end, new_money, new_transport):
        seen = []
        for lon in range(min(start.longitude, end.longitude), max(start.longitude, end.longitude)+1):
            for lat in range (min(start.latitude, end.latitude), max(start.latitude, end.latitude)+1):
...
def plot_graph(graph):
    ...
    nx.draw(graph.graph, pos, node_size=10, edge_color=weights, width=1, edge_cmap=plt.cm.Blues, with_labels=False)
